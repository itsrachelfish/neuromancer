module.exports = {
  say: "<message> identical to echo",
  wflogin: "sends the wetfish nickserv 'login' via pm",
  join: "<channel> makes the bot join the specified channel",
  part: "<opt: channel> makes the bot part the specified channel, or the current channel if none given",
  ctcp: "<target> <type> <message> makes the bot send a ctcp message",
  load: "<type>.<name> makes the bot load the specified module",
  unload: "<type>.<name> makes the bot unload the specified module",
  reload: "<type>.<name> makes the bot reload the specified module"
}